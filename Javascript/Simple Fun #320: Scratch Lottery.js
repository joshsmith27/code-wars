...
function scratch(lottery){
  ...
  var number = 0;
  var arrToCheck = [];
  for (var x in lottery){
    arrToCheck = lottery[x].split(' ');
    if(arrToCheck[0] === arrToCheck[1] && arrToCheck[0] === arrToCheck[2]){
      number = parseInt(arrToCheck[3]);
...
// best answer
function scratch(a) {
  return a.reduce((s, t) => s + (/([a-z]+) \1 \1/.test(t) ? +t.split` `[3] : 0), 0);
};
